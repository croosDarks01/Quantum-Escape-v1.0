...
new gmsgWeaponList, setrum, item_cyclone
 
const SECONDARY_WEAPONS_BIT_SUM = (1<<CSW_AK47)|(1<<CSW_ELITE)|(1<<CSW_FIVESEVEN)|(1<<CSW_USP)|(1<<CSW_GLOCK18)|(1<<CSW_DEAGLE)
new const WEAPONENTNAMES[][] = { "", "weapon_p228", "", "weapon_scout", "weapon_hegrenade", "weapon_xm1014", "weapon_c4", "weapon_mac10", "weapon_aug", "weapon_smokegrenade", "weapon_elite", "weapon_fiveseven", "weapon_ump45", "weapon_sg550", WEAPON_BASE, "weapon_famas", "weapon_usp", "weapon_glock18", "weapon_awp", "weapon_mp5navy", "weapon_m249",
...
public plugin_init()
{
    ...
    RegisterHam(Ham_Item_AddToPlayer, WEAPON_BASE, "fw_sfpistol_AddToPlayer")
    for (new i = 1; i < sizeof WEAPONENTNAMES; i++)
    if (WEAPONENTNAMES[i][0]) RegisterHam(Ham_Item_Deploy, WEAPONENTNAMES[i], "fw_Item_Deploy_Post", 1)
    RegisterHam(Ham_Item_PostFrame, WEAPON_BASE, "sfpistol_ItemPostFrame")
    RegisterHam(Ham_Weapon_Reload, WEAPON_BASE, "sfpistol_Reload")
    RegisterHam(Ham_Weapon_Reload, WEAPON_BASE, "sfpistol_Reload_Post", 1)
    RegisterHam(Ham_Weapon_PrimaryAttack, WEAPON_BASE, "fw_sfpistol_PrimaryAttack")
    RegisterHam(Ham_Weapon_PrimaryAttack, WEAPON_BASE, "fw_sfpistol_PrimaryAttack_Post", 1)
    RegisterHam(Ham_Weapon_WeaponIdle, WEAPON_BASE, "fw_cycloneidleanim", 1)
    ...
    item_cyclone = ze_register_item("Cyclone", 20, 0)
    ...
    cvar_dmg_sfpistol = register_cvar("ze_sfpistol_dmg", "1.0")
    cvar_clip_sfpistol = register_cvar("ze_sfpistol_clip", "50")
    cvar_sfpistol_ammo = register_cvar("ze_sfpistol_ammo", "200")
...
public plugin_precache()
{
    ...
    precache_generic("sprites/640hud104_3.spr")
...
{
    register_native("give_sfpistol", "native_give_sfpistol", 1)
}
...
public fw_SetModel(entity, model[])
{
    ...
    if(equal(model, "models/w_ak47.mdl"))
    {
        ...
        iStoredAugID = find_ent_by_owner(ENG_NULLENT, WEAPON_BASE, entity)
...
{
    drop_weapons(id, 2)
    new iWep2 = fm_give_item(id,WEAPON_BASE)
    if( iWep2 > 0 )
    {
        cs_set_weapon_ammo(iWep2, get_pcvar_num(cvar_clip_sfpistol))
        cs_set_user_bpammo (id, CSW_AK47, get_pcvar_num(cvar_sfpistol_ammo))   
       
        set_weapons_timeidle(id, CSW_AK47, 1.0)
...
public fw_CmdStart(id, uc_handle, seed)
{
    if(!is_user_alive(id) || !is_user_connected(id))
        return
    if(get_user_weapon(id) != CSW_AK47 || !g_has_sfpistol[id])
        return
    ...
    if(!(pev(id, pev_oldbuttons) & IN_ATTACK))
    {
        if(zz[id] && g_clip_ammo[id])
        {
            ...
            set_weapons_timeidle(id, CSW_AK47, 0.5)
...
public fw_cycloneidleanim(Weapon)
{
    ...
    if(!is_user_alive(id) || ze_is_user_zombie(id) || !g_has_sfpistol[id] || get_user_weapon(id) != CSW_AK47)
        return HAM_IGNORED;
...
public CurrentWeapon(id)
{
     ...
     if(read_data(2) != CSW_AK47 || !g_has_sfpistol[id])
          return
...
{
    if(!is_user_alive(Player) || (get_user_weapon(Player) != CSW_AK47 || !g_has_sfpistol[Player]))
        return FMRES_IGNORED
...
public fw_sfpistol_PrimaryAttack_Post(Weapon)
{
    ...
    if(g_has_sfpistol[Player])
    {
        ...
        set_weapons_timeidle(Player, CSW_AK47, 0.075)
...
public sfpistol_Reload_Post(weapon_entity)
{
    ...
    set_weapons_timeidle(id, CSW_AK47, sfpistol_RELOAD_TIME)
