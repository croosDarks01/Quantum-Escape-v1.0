...
public Cmd_Knife(id)
{
	...
	{
		CC_SendMessage(id, "%L %L", id, "KM_CHAT_PREFIX", id, "KM_IS_ZOMBIE")
		return PLUGIN_HANDLED
	}
...
select_knife(id, iKnife)
{
    ...
    // Check if the selected knife is Dreadnova (ID 6)
    if (iKnife == 6)
    {
        zp_give_user_dreadnova(id);
    }
    else // Selected knife is not Dreadnova
    {
        // Check if the player has Dreadnova knife
        if (zp_get_user_dreadnova(id) == 1)
        {
            zp_delete_user_dreadnova(id); // Delete Dreadnova knife only if the player has it
        }
        if (get_user_weapon(id) == CSW_KNIFE)
        // Refresh the knife model if it's not Dreadnova
        refresh_knife_model(id); 
    }

    // Sending messages and playing sounds remain unchanged
    if(is_user_alive(id))
    {   
...
bool:has_knife_access(const id, const iKnife)
{
	...
	if(g_bRankSystem)
	{
		if(eKnife[LEVEL] && ze_get_user_level(id) < eKnife[LEVEL])
		{
			return false
		}

		if(eKnife[XP] && ze_get_user_xp(id) < eKnife[XP])
		{
			return false
		}
	}
...
public plugin_natives()
{
	...
	register_native("crxknives_knife_show_menu",     "_native_knife_show_menu")
...
public _native_knife_show_menu(const plugin_id, const num_params)
{
	new id = get_param(1); // Get the player's ID
	Cmd_Knife(id)
	return true
}
