...
{
	new const szZombieName[] = "Husk Zombie"
	new const szZombieDesc[] = "-= Special =-"
...
{
	if (ze_zclass_get_current(id) == g_Husk)
	{
...
public fw_PlayerPreThink(id)
{
	...
	if(ze_is_user_zombie(id) && (ze_zclass_get_current(id) == g_Husk))
	{
...
public husk_touch(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, get_pcvar_float(cvar_fireradius))) != 0)
	{
		...
		if (!is_user_valid_alive(victim) || ze_is_user_zombie(victim) || !get_pcvar_num(cvar_firesurvivor))
			continue;
...
public burning_flame(taskid)
{
	...
	if (ze_is_user_zombie(ID_BURN) || (flags & FL_INWATER) || g_burning_duration[ID_BURN] < 1)
	{
		// Smoke sprite
